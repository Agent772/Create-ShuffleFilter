...
public class ShuffleFilterUtil {
     ...
     * @return ItemStack to deploy, or empty if not found
     */
    public static ItemStack selectItemCascading(
        ...
        // This respects the filter's intent while maintaining early exit performance
        ...
        sortedEntries.sort((a, b) -> {
            ...
            // Tiebreaker: maintain original index order
    ...
    private static ItemStack tryExtractEntry(
        ...
        if (item instanceof BaseShuffleFilterItem) {
            // Get configuration from the stored filter (not from inventory!)
            ...
            // Determine mode for nested filter
        ...
        if (item instanceof FilterItem) {
            // Create FilterItemStack wrapper for matching (uses stored configuration)
     ...
     * Performance: ~10ns
     * 
     * @param blockList Block list to select from
     * @param useWeighted Whether to use weighted selection
     * @param world Level for random number generation
     * @return Selected BlockEntry, or null if list is empty
     */
    public static ShuffleBlockList.BlockEntry selectEntry(ShuffleBlockList blockList, boolean useWeighted, Level world) {
        ...
        if (useWeighted) {
            // Weighted selection with accumulated probability
    ...
    /**
     * Select block for roller (NO extraction - roller extracts separately)
     * Returns the ItemStack type to place, checking inventory availability
     * 
     * @param blockList Block list to select from  
     * @param useWeighted Whether to use weighted selection
     * @param world Level for random generation
     * @param inv Contraption inventory to check availability
     * @return ItemStack representing which block to place, or EMPTY if none available
     */
    public static ItemStack selectBlockForRoller(
        ...
        // Try selected entry first
        ShuffleBlockList.BlockEntry selectedEntry = selectEntry(blockList, useWeighted, world);
        if (selectedEntry != null) {
            ItemStack configured = selectedEntry.getItemStack();
            if (!configured.isEmpty() && hasItemInInventory(inv, configured.getItem())) {
                return configured; // Return WITHOUT extracting
            }
        }

        // Fallback: try other entries (sorted by weight)
        List<ShuffleBlockList.BlockEntry> sortedEntries = new ArrayList<>(blockList.blocks());
        sortedEntries.sort((a, b) -> {
            int weightCmp = Float.compare(b.weight(), a.weight());
            if (weightCmp != 0) return weightCmp;
        ...
        for (ShuffleBlockList.BlockEntry entry : sortedEntries) {
            if (entry == selectedEntry) continue; // Already tried
        ...
        return ItemStack.EMPTY; // No blocks available in inventory
    }
}
