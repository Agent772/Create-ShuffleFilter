...
public class WeightEditBox extends EditBox {
    ...
    public boolean mouseScrolled(double mouseX, double mouseY, double scrollX, double scrollY) {
        if (this.isHoveredOrFocused()) {
            ...
            int newWeight = Math.max(0, Math.min(100, currentWeight + delta));
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        ...
        if (result && button == 0) { // Left click
            long currentTime = System.currentTimeMillis();
            long timeSinceLastClick = currentTime - lastClickTime;
            
            // Double-click detected - move cursor to end
    ...
    public void setFocused(boolean focused) {
        ...
        // When focus is lost, validate and notify
    ...
    /**
     * Validate current value and notify listener
     */
